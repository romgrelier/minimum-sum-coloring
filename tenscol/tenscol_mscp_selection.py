...
for i in pbar:
    ...
    if begin != 0:
        ...
        frozen_kappa += sum(A * left_concentration)
        frozen_varpi += sum(notA * left_concentration)
        frozen_sum_l2_regul += sum(left_onehot ** 2.0)
    ...
    if begin + mask_size < d:
        ...
        frozen_kappa += sum(A * right_concentration)
        frozen_varpi += sum(notA * right_concentration)
        frozen_sum_l2_regul += sum(right_onehot ** 2.0)
    ...
    for j in range(100):
        ...
        kappa = sum(A * selection_concentration) + frozen_kappa
        varpi = sum(notA * selection_concentration) + frozen_varpi
        sum_l2_regul = sum(selection ** 2.0) + frozen_sum_l2_regul
